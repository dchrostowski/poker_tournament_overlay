...
function renderLinks(tlist) {
    ...
    const links = tlist.map((tinfo) => {
        ...
        const href1=`/overlays/${site}/?uid=${uniqueId}&widgetType=ticker`
        const href2=`/overlays/${site}/?uid=${uniqueId}&widgetType=table`
        return (
            <ul>
            <li>
...
function OverlayHome(props) {
    const siteChosen = props.site === 'stockpokeronline.com' || props.site === 'roundercasino.com'
